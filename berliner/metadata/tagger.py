...
def build_metadata(cfg: dict) -> dict:
    t0 = time.time()
    meta = cfg["metadata"]; paths = meta["paths"]
    pdf_dir = Path(paths["pdf_dir"])
    chunks_dir = Path(paths["chunks_dir"])
    sums_dir = Path(paths["summaries_dir"])
    out_dir = Path(paths["enriched_dir"]); out_dir.mkdir(parents=True, exist_ok=True)
    ...
    # quick PDF map (skip unparseable filenames)
    pdf_map = {}
    ...
    counts = dict(issues=0, chunks=0, summaries=0, written=0,
    ...
    with out_jsonl.open("w", encoding="utf-8") as w:
        for chunk_file in sorted(chunks_dir.glob("*.jsonl")):
            ...
            try:
                ...
                # try reading first line to find a hint
                try:
            ...
            for idx, (chunk, summ) in enumerate(zip(iter_jsonl(chunk_file), iter_jsonl(sum_file))):
                n += 1; counts["chunks"] += 1; counts["summaries"] += 1
                c_txt = chunk.get("text", "") or ""
                s_txt = summ.get("summary", summ.get("text", "")) or ""
                ...
                rec = {
                    ...
                    "summarizer_params": summ.get("params", {"num_beams": meta["summarizer_defaults"]["num_beams"]}),
    ...
    schema = {
        ...
        "fields": {
            "issue_id":"str","issue_date":"YYYY-MM?","year":"int?","month":"int?",
            "source_pdf_path":"str?","parser_version":"str",
            "chunk_id":"str","chunk_index":"int","chunk_word_count":"int","chunk_text_hash":"str?",
            "summary_text":"str","summary_word_count":"int","summarizer_model":"str","summarizer_params":"object",
            "summary_text_hash":"str?","runtime_s_extract":"float?","runtime_s_summarize":"float?",
            "created_at":"datetime","lang":"str","file_size_bytes":"int?"
        }
    }
